...
const VideoModal = ({
  ...
  // Sources parameter is available for future use
  console.debug("VideoModal sources available:", _sources);
  ...
  const relatedVideos = useMemo(() => {
    ...
    if (related.length < 100) {
      const others = allVideos.filter(
        (v) => v.id !== video.id && !related.includes(v)
      );
                ...
                {relatedVideos.map((rv) => (
                  ...
                  >
                    <div className="related-video-thumb">
                      ...
                      <span className="related-video-duration">
                        {rv.duration || "00:00"}
                      </span>
                    </div>
                    <div className="related-video-info">
                      <div className="related-video-title" title={rv.title}>
                        {rv.title}
                      </div>
                      <div className="related-video-meta">
                        <span>
                          <FaEye /> {formatViews(rv.views)}
                        </span>
                        <span>
                          <FaStar /> {rv.rating || 0}%
                        </span>
