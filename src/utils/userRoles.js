...
// Force update to new default admin PIN
...
// Force update inactivityTimeout to 1 minute for all users
(function forceUpdateAutoLockTimeout() {
  const settings = JSON.parse(localStorage.getItem(AUTO_LOCK_SETTINGS_KEY) || '{}');
  if (settings.inactivityTimeout !== 60000) {
    settings.inactivityTimeout = 60000;
...
export const getSectionPin = (sectionName) => {
  const sectionPins = JSON.parse(localStorage.getItem(SECTION_PINS_KEY) || "{}");
...
export const startSession = (sectionName) => {
  const sessionData = JSON.parse(localStorage.getItem(SESSION_DATA_KEY) || "{}");
...
export const isSessionValid = (sectionName) => {
  ...
  return sessionAge < settings.sessionTimeout && inactivityTime < settings.inactivityTimeout;
};
...
export const getRemainingSessionTime = (sectionName) => {
  ...
  const inactivityRemaining = Math.max(0, settings.inactivityTimeout - inactivityTime);
